...
mod assets;
mod config;
mod entities; // should this be called in main? focus on building in blocked out files
mod machinery;
mod office;
...
struct GameState {
    // Should have
    // Assets
    // Utilities - buttons etc shown on screen
    // Config
    // Environment - this needs to contain all loose things including gloop / upgrades like
    // trees and buses etc
    // Machinery - engine, gears & overall upgrades
    // Office - manages all gloops & gloop based upgrades
    assets: Assets,
    config: Config,
    engine: Engine, // find a way to store game items without adding them all here
    office: Office,
    environment: Environment, // track overall earned gloop when start taking away for using
    extractor1: GloopExtractor,
    gloop_label: Text, // rotations: Text,
    smog_level: Text,
    start_engine: StartEngineButton,
    spare_gloop: f32,
    gloops: Vec<Gloop>, // put in environment as not being utilised here
    add_glooper_button: AddGlooperButton,
}
...
impl GameState {
    fn new(ctx: &mut Context) -> tetra::Result<GameState> {
        ...
        let add_glp_button = AddGlooperButton::new(&assets);
        ...
        Ok(GameState {
            ...
            gloops: Vec::new(),
            spare_gloop: 0.,
            add_glooper_button: add_glp_button,
        })
    }
}
...
impl State for GameState {
    fn update(&mut self, ctx: &mut Context) -> tetra::Result {
        ...
        if is_mouse_button_pressed(ctx, MouseButton::Left) {
            ...
            if self.add_glooper_button.bounds().contains_point(mouse_pos) {
                println!("Add glooper");
                self.office.add_glooper(Glooper::new(
                    &self.assets,
                    Vec2::new(32., self.config.window_height - 70.),
                ));
            }
        }
        ...
        if self.extractor1.gear.rotation >= (2. * PI) {
            ...
            if self.extractor1.gloop_to_drop > 0. {
                for _ in 1..=self.extractor1.gloop_to_drop as usize {
                    // stop gloop overlapping when drawn in here
                    let y_change = spread.sample(&mut rng) as f32;

                    // check this doesn't overlap a block on creation!
                    let new_gloop = Gloop::new(
                        &self.assets,
                        Vec2::new(
                            self.config.window_width / 2. + y_change.floor(),
                            self.config.window_height - 512.,
                        ),
                    );
                    self.gloops.push(new_gloop);
                }
                self.extractor1.gloop_to_drop = 0.;
            }
        }

        let gloop_list = self.gloops.clone();

        for glp in &mut self.gloops {
            // need to check if it is moving
            glp.update_position(ctx, &gloop_list);
        }
    ...
    fn draw(&mut self, ctx: &mut Context) -> tetra::Result {
        ...
        self.engine.texture.draw(
            ctx,
            DrawParams::new()
                .position(Vec2::new(256., self.config.window_height - 112.))
                .scale(Vec2::new(5., 5.))
                .origin(Vec2::new(
        ...
        if self.environment.gloop > 105. {
            println!("Gloop {}", self.environment.gloop);
            self.add_glooper_button.texture.draw(
                ctx,
                DrawParams::new()
                    .position(Vec2::new(self.config.window_width - 156., 300.))
                    .scale(Vec2::new(10., 10.))
        ...
        for glooper in &self.office.gloopers {
            glooper.texture.draw(
                ctx,
                DrawParams::new()
                    .position(glooper.position)
                    .scale(Vec2::new(glooper.scale, glooper.scale)),
            )
        }
...
struct AddGlooperButton {
    texture: Texture,
}

impl AddGlooperButton {
    fn new(assets: &Assets) -> AddGlooperButton {
        AddGlooperButton {
    ...
    fn bounds(&self) -> Rectangle {
        Rectangle::new(
            WIN_WIDTH - 56. - (self.texture.width() as f32 * 5.) / 2.,
            WIN_HEIGHT - 312. - ((self.texture.height() as f32 * 5.) / 2.),
            self.texture.width() as f32 * 5.,
            self.texture.height() as f32 * 5.,
        )
    }
